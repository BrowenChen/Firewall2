...
class log_buffer:
    ...
    def parse_input(self, pkt, direction, seqno, syn, ack):
        ...
        if direction == self.incoming_direction:
            print("SEQNO")
            print seqno
            print ("CURRENT INCOMING SEQNO")
            print self.incoming_seqno
            if self.incoming_seqno == None:
                self.incoming_seqno = seqno
            print("LOOK HERE")
            print (seqno - self.incoming_seqno)
            if seqno == self.incoming_seqno:
                print "PACKET LENGTH"
                print len(pkt)
                print "SYN"
                print syn
    ...
    def http_create_log(self, http_request, http_response):
        ...
        for line in http_res_list:
            ...
            line_list = map(lambda x:x.strip(), line_ )
